...
async function createTicketChannel(interaction, ticketType) {
    ...
    try {
        ...
        const ticketEmbed = new EmbedBuilder()
            ...
            .setColor(0x2B017F);
        await channel.send({ content: `${user.toString()} <@&${supportRoleId}>`, embeds: [ticketEmbed] });
...
client.on('messageCreate', async (message) => {
    ...
    if (command === 'jail') {
        ...
        const confirmRow = createConfirmationButtons('jail');
        const confirmMsg = await message.reply({ embeds: [confirmEmbed], components: [confirmRow] });
        pendingActions.set(confirmMsg.id, { action: 'jail', targetUserId: userId, targetUserTag: targetMember.user.tag, reason: reason, originalCommandAuthorId: message.author.id, originalCommandId: message.id, guildId: message.guild.id, channelId: message.channel.id });
    }
    ...
    if (command === 'ban') {
        ...
        const confirmRow = createConfirmationButtons('ban');
        const confirmMsg = await message.reply({ embeds: [confirmEmbed], components: [confirmRow] });
        pendingActions.set(confirmMsg.id, { action: 'ban', targetUserId: userId, targetUserTag: targetMember.user.tag, reason: reason, originalCommandAuthorId: message.author.id, originalCommandId: message.id, guildId: message.guild.id, channelId: message.channel.id });
    }
    ...
    if (command === 'unban') {
        ...
        try {
            ...
            const confirmRow = createConfirmationButtons('unban');
            const confirmMsg = await message.reply({ embeds: [confirmEmbed], components: [confirmRow] });
            pendingActions.set(confirmMsg.id, { action: 'unban', targetUserId: userId, targetUserTag: unbannedUser.tag, reason: 'No reason provided', originalCommandAuthorId: message.author.id, originalCommandId: message.id, guildId: message.guild.id, channelId: message.channel.id });
    ...
    if (command === 'iunban') {
        ...
        try {
            ...
            const confirmRow = createConfirmationButtons('iunban');
            const confirmMsg = await message.reply({ embeds: [confirmEmbed], components: [confirmRow] });
            pendingActions.set(confirmMsg.id, { action: 'iunban', targetUserId: userId, targetUserTag: unbannedUser.tag, reason: 'No reason provided', originalCommandAuthorId: message.author.id, originalCommandId: message.id, guildId: message.guild.id, channelId: message.channel.id });
        } catch (error) { await message.reply({ embeds: [createErrorEmbed('Error', 'Failed to find banned user.')] }); }
    }
    ...
    if (command === 'kick') {
        ...
        const confirmRow = createConfirmationButtons('kick');
        const confirmMsg = await message.reply({ embeds: [confirmEmbed], components: [confirmRow] });
        pendingActions.set(confirmMsg.id, { action: 'kick', targetUserId: userId, targetUserTag: targetMember.user.tag, reason: reason, originalCommandAuthorId: message.author.id, originalCommandId: message.id, guildId: message.guild.id, channelId: message.channel.id });
    }
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.customId === 'confirm_ban' || interaction.customId === 'confirm_kick' || interaction.customId === 'confirm_jail' || interaction.customId === 'confirm_unban' || interaction.customId === 'confirm_iunban') {
        const action = interaction.customId.split('_')[1];
        const pendingData = pendingActions.get(interaction.message.id);
